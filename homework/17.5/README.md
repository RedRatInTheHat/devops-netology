...
Для создания YDB, s3 backet и сервисного аккаунта создана директория s3-with-lock: [main.tf](terraform/s3-with-lock/main.tf).<br/>
Директория в своём названии немного врёт, потому что таблицы с lockID нет. Точнее, для неё подготовлена структура, но Yandex любезно предоставил совместимость с AWS, так что, насколько я понимаю, без данных из AWS создание таблицы не работает. Спасибо большое, очень удобно.

Для проверки руками была создана таблица `tf-locks` и выполнено задание из предыдущего ДЗ, [17.4.8](../17.4/README.md), код размещён там же, в директории `roots`. Состояния были успешно созданы и использованы, блокировки соблюдались.

Не то, чтобы пруф, но могу показать оба state'а. :D
